...
function generateCode(graph: IGraph, namespaceName?: string): string {
    ...
    const eventClassBlocks: string[] = [];
    const eventHubEventLines: string[] = [];
    const eventHubTriggerLines: string[] = [];
    const usedOnEventNames = new Set<string>();
    let nameCounter = 1;
...
${memberLines.join("\n")}
    ...
    for (const eventNode of eventNodes) {
        const eventNameRaw = String(eventNode.properties?.name || eventNode.id || `Event${nameCounter}`);
        const eventName = toPascalCase(eventNameRaw, `Event${nameCounter++}`);
        const payloadClassName = ensureUniqueTypeName(`${eventName}Event`);
        const variants = getChildVariants(eventNode.id);

        const usedPropertyNames = new Set<string>(["type", "Type"]);
        const memberLines: string[] = [
            `    public string type = ${JSON.stringify(eventNameRaw)};`,
        ];
        for (const variantNode of variants) {
            const fieldNameRaw = String(variantNode.properties?.name || variantNode.id || "field");
            const csName = ensureUniquePropertyName(usedPropertyNames, toPropertyName(fieldNameRaw, `field${nameCounter++}`));
            const csType = extractVariantCsharpType(eventName, variantNode, fieldNameRaw);
            const finalType = isCsharpValueType(csType) ? `${csType}?` : csType;
        ...
        const classDoc = docCommentLines(eventNode.properties?.description, "");
        const classDocPrefix = classDoc.length > 0 ? `${classDoc.join("\n")}\n` : "";
        eventClassBlocks.push(`${classDocPrefix}[Serializable]
public class ${payloadClassName}
{
    public const string Type = ${JSON.stringify(eventNameRaw)};

${memberLines.join("\n")}
}`);

        const onBaseName = eventName === "On" || /^On[A-Z0-9]/.test(eventName) ? eventName : `On${eventName}`;
        const onName = ensureUniquePropertyName(usedOnEventNames, toIdentifier(onBaseName, `OnEvent${nameCounter++}`));
        eventHubEventLines.push(`    public event Action<${payloadClassName}> ${onName};`);
        eventHubTriggerLines.push(`    public void Trigger(${payloadClassName} e)\n    {\n        ${onName}?.Invoke(e);\n    }`);
    }

    if (eventHubEventLines.length > 0) {
        const hubClassName = ensureUniqueTypeName(`${toPascalCase(graph.name, "Graph")}Events`);
        const hubMembers = [...eventHubEventLines];
        if (eventHubTriggerLines.length > 0) {
            hubMembers.push("", ...eventHubTriggerLines);
        }
        eventClassBlocks.push(`// On-prefixed events for this world
public class ${hubClassName}
{
${hubMembers.join("\n")}
}`);
    }
    ...
    if (contextNodes.length === 0 && eventNodes.length === 0) {
        code += `\n// No Context/World/Event nodes were found in this graph.\n`;
        return code;
    }
    ...
    if (eventClassBlocks.length > 0) {
        blocks.push(`// Event classes generated from Event nodes\n${eventClassBlocks.join("\n\n")}`);
    }
    if (contextClassBlocks.length > 0) {
